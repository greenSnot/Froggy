...
export function useWorkspaceEvents({ initial_blocks_offset }) {
    ...
    const [blocks_offset, set_blocks_offset] = useState(initial_blocks_offset);
    const workspace_on_drag = useCallback((e) => {
        const workspace_data = workspace_drag_start_data_ref.current;
        const new_offset = {
            x: Math.round(workspace_data.workspace_global_x +
                e.pageX -
                workspace_data.mouse_global_x),
            y: Math.round(workspace_data.workspace_global_y +
                e.pageY -
